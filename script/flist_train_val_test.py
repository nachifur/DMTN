...
def gen_flist_train_val_test(flist, train_val_test_path, train_val_test_ratio, SEED, id_list):
    random.seed(SEED)
    # get flist
    ...
    if sum(train_val_test_ratio) == 10:
        for i in range(3):
            if train_val_test_ratio[i] > 0:
                sum_ = sum_ + \
                    np.int(np.floor(train_val_test_ratio[i]*files_num/10))
                if i == 2:
                ...
                # save
                np.savetxt(train_val_test_path[i],
                           images_train_val_test[i], fmt='%s')
            else:
                sum_ = sum_ + \
                    np.int(np.floor(train_val_test_ratio[i]*files_num/10))
                i_list.append(sum_)
